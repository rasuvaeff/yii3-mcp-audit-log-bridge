...
Каждый `tools/call` (инструменты атрибутов, операции с мостом OpenAPI,
 ...
 | изменения | одно поле на каждый аргумент инструмента + `mcp.outcome` (`success`/`error`), `mcp.duration_ms`, `mcp.error` (сообщение, при сбое) |
 | метаданные | requestId = идентификатор сеанса, userAgent = имя клиента |

 Сбои регистрируются и **перегенерируются** — конверт ошибки MCP, который видит агент
, не изменяется, а вызов, который завершился сбоем, по-прежнему проверяется. @@ЛИНИЯ@@
