...
- **Отказы session budget невидимы.** yii3-mcp сам ставит свой
  `SessionBudgetInterceptor` самым внешним — снаружи любых перечисленных вами
  интерцепторов. Вызов, отклонённый бюджетом сессии, не доходит до цепочки
  интерцепторов и не порождает события аудита: исчерпанный бюджет выглядит
  как замолчавший агент, а не как всплеск строк `rejected`.
- **Отказы session-ownership невидимы** (yii3-mcp `^2.0`). Ядро привязывает
  каждую сессию к создавшему её MCP-клиенту и отклоняет вызов по чужой или
  безвладельческой сессии (SDK-шейп 404 из `McpAction`,
  `SessionOwnershipException` из `InterceptingReferenceHandler`) **до**
  запуска цепочки интерцепторов. Отклонённый по ownership вызов не порождает
  события аудита — попытки перехвата сессии видны только в логах приложения
  и веб-сервера, но не в аудит-журнале.
...
На yii3-mcp `^2.0` эта атрибуция ещё сильнее: ядро делает владельца сессии
неизменяемым — он фиксируется один раз при `initialize` и никогда не
переприсваивается, — поэтому `mcp.client_id` в аудит-записях одной сессии
больше не может смениться посреди сессии. Коду этого моста ничего не нужно —
гарантию даёт ядро.
